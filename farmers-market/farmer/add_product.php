...
$errors = [];
$old = ['product_name' => '', 'category_id' => '', 'description' => '', 'price' => '', 'unit' => 'kg', 'stock_quantity' => '', 'is_organic' => 0];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($old['description']) < 10) $errors['description'] = 'Please provide a description of at least 10 characters.';
    if (!is_numeric($old['price']) || $old['price'] <= 0) $errors['price'] = 'Please enter a valid price greater than 0.';
    if ($old['unit'] === '') $errors['unit'] = 'Please enter a unit (e.g. kg, dozen, bunch).';
    if (!ctype_digit((string)$old['stock_quantity']) || (int)$old['stock_quantity'] < 0) $errors['stock_quantity'] = 'Please enter a valid stock quantity (0 or more).';
    ...
    if (empty($errors)) {
        $insert = $pdo->prepare("INSERT INTO products (farmer_id, category_id, product_name, description, price, unit, stock_quantity, is_organic, image)
                                  VALUES (?,?,?,?,?,?,?,?,?)");
        $insert->execute([
            current_user_id(), $old['category_id'], $old['product_name'], $old['description'],
            $old['price'], $old['unit'], $old['stock_quantity'], $old['is_organic'], $imageName
        ]);
...
$pageTitle = 'Add Product | Farmer Dashboard';
...
<section class="section">
  <div class="dashboard-wrap">
    <aside class="sidebar" aria-label="Farmer navigation">
      ...
      <a href="<?php echo BASE_URL; ?>/farmer/add_product.php" class="active">Add New Product</a>
      <a href="<?php echo BASE_URL; ?>/farmer/orders.php">My Product Orders</a>
    </aside>

    <div class="dashboard-content">
      <h1>Add New Product</h1>
      <form class="form-box" method="POST" action="<?php echo BASE_URL; ?>/farmer/add_product.php" enctype="multipart/form-data" data-validate novalidate style="max-width:100%;">
        <?php echo csrf_field(); ?>
        ...
        <div class="form-group">
          ...
          <?php if (!empty($errors['category_id'])): ?><div class="error-text"><?php echo clean($errors['category_id']); ?></div><?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" id="unit" name="unit" placeholder="kg, dozen, bunch, 500g jar..." required value="<?php echo clean($old['unit']); ?>">
          <?php if (!empty($errors['unit'])): ?><div class="error-text"><?php echo clean($errors['unit']); ?></div><?php endif; ?>
        </div>
        ...
        <div class="form-group">
          <label><input type="checkbox" name="is_organic" style="width:auto;display:inline;" <?php echo $old['is_organic']?'checked':''; ?>> This product is certified organic</label>
        </div>

        <div class="form-group">
          <label for="image">Product Image (jpg, png or webp, max 2MB)</label>
